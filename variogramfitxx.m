function [a,c,n,S] = variogramfitxx(h,gammaexp,a0,c0,numobs,varargin)
% [a,c,n] = variogramfit(h,gammaexp,200,15000,[],'solver','fminsearchbnd','model','pentaspherical','nugget',0,'plotit',true)
...
if params.plotit
    ...
    vvv = max(gammaexp);
    if vvv>=10000 && vvv<=20000
        vvv = 20000;
    elseif vvv>20000
        vvv = max(gammaexp)+0.1*max(gammaexp);   
    else
       vvv =  vvv;
    end
    axis([0 max(h)+0.4*max(h) 0 vvv])
